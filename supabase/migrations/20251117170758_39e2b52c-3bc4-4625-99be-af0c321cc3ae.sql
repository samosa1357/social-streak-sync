...
-- Issue 2: Make avatars bucket private and add privacy-respecting policies
...
-- Add privacy-aware SELECT policy for avatars
CREATE POLICY "Avatar visibility follows profile privacy" 
ON storage.objects FOR SELECT
USING (
  bucket_id = 'avatars' AND (
    -- Owner can always view their own avatar
    ...
    -- Others can view if they are accepted followers
    EXISTS (
...
-- Re-add upload/update/delete policies for avatars
...
-- Issue 3: Add documentation and validation to security definer functions
-- Update handle_new_user with better documentation and validation
...
-- Update create_follow_notification with better documentation and validation
...
-- This function is ONLY callable by the database trigger, not by users directly.
BEGIN
  -- Validate that follower exists (basic sanity check)
